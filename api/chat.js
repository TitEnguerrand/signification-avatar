...
async function vectorSearch(query, openaiKey, sbUrl, sbKey, topK) {
  topK = topK || 10;
  var emb = await getEmbedding(query, openaiKey);
  var r = await fetch(sbUrl + '/rest/v1/rpc/match_chunks', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'apikey': sbKey, 'Authorization': 'Bearer ' + sbKey },
    body: JSON.stringify({ query_embedding: emb, match_count: topK })
  });
  if (!r.ok) throw new Error('Supabase: ' + (await r.text()));
  var results = await r.json();
  return results.map(function(x) { return { source: x.source, text: x.body }; });
}

function writeSSE(res, obj) {
  var s = JSON.stringify(obj);
  var safe = s.replace(/[\u0080-\uffff]/g, function(c) {
    return '\\u' + ('0000' + c.charCodeAt(0).toString(16)).slice(-4);
  });
  res.write(Buffer.from('data: ' + safe + '\n\n', 'ascii'));
}
        ...
        if (line.startsWith("data: ")) {
          ...
          try {
            ...
            if (event.type === "content_block_delta" && event.delta && event.delta.text) {
              writeSSE(res, { text: event.delta.text });
            }
          } catch (e) {}
        }
      }
